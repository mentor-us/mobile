...
import {PinModel} from "~/models/message";
...
const mockdata: PinModel = {
  ...
  messagePin: {
    id: "2",
    content: "18h30 thứ tư mỗi 2 tuần chúng ta có cuộc họp với thầy",
...
const Pin = ({data = mockdata}: Props) => {
  return (
    <TouchableOpacity style={styles.container}>
      ...
      {/* Message pin */}
      <View style={styles.massagePin}>
        <Text numberOfLines={1} style={styles.pinTitle}>
          {data.messagePin.sender}
        </Text>
        <Text numberOfLines={2}>{data.messagePin.content}</Text>
      </View>
    </TouchableOpacity>
  );
};
